...
function civicrm_api3_geostelsel_update($api_params) {
  ...
  $dao =CRM_Core_DAO::executeQuery("SELECT `{$postcode_table}`.`{$gemeente_field}` AS `gemeente`,
                                            ...
                                            (SELECT provincie FROM civicrm_postcodenl WHERE
`civicrm_postcodenl`.`postcode_nr` = SUBSTR(REPLACE(`civicrm_address`.`postal_code`, ' ', ''), 1, 4) AND BINARY `civicrm_postcodenl`.`postcode_letter` = BINARY SUBSTR(REPLACE(`civicrm_address`.`postal_code`, ' ', ''), 5, 2) LIMIT 1) as provincie
                                    FROM `{$postcode_table}`
  ...
  while($dao->fetch()) {
    $count ++;
    $repo->updateContact($dao->contact_id, $dao->gemeente, $dao->provincie);
  }
