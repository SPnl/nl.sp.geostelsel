...
class CRM_Geostelsel_BAO_Toegangsgegevens {
  ...
  public static function getWhereForAfddeling($afdeling_id, &$tables, &$whereTables) {
    ...
    /*if (stristr($contact_sub_type, CRM_Core_DAO::VALUE_SEPARATOR."SP_Landelijk".CRM_Core_DAO::VALUE_SEPARATOR)) {
      ...
      contact_a.id IN (
        SELECT geostelsel.entity_id 
        FROM {$table} geostelsel 
        INNER JOIN {$membership_table} membership_access ON geostelsel.entity_id = membership_access.contact_id
        WHERE
        (geostelsel.`{$afdeling}` = {$afdeling_id} OR geostelsel.`{$regio}` = {$afdeling_id} OR geostelsel.`{$provincie}` = {$afdeling_id})
        AND membership_access.membership_type_id IN ({$mtype_ids}) 
        AND (membership_access.status_id IN ({$mstatus_ids}) OR (membership_access.status_id = '{$membership_type->getDeceasedStatusId()}' AND membership_access.end_date >= NOW() - INTERVAL 3 MONTH))
      )
      OR contact_a.id = {$afdeling_id}";
    }*/
  ...
  public static function getGroupOptions() {
    $groupHierarchy = CRM_Core_PseudoConstant::nestedGroup(FALSE);
    $return[] = '';
